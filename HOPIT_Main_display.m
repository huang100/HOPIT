...
Pars.n_iter=10;                                                          % Number of simulation iterations
...
%     Data.Vignette=[ones(Pars.data_point,1),Vignette]; % It is necessary to add a constant term since Vignette only take values 1-4
    ...
    if converge==1                                                              % converge==1 if all above models converge
        ...
        coef(:,:,n_converge)=[b1(1:2) b2(1:2) b3(1:2) b6(1:2) b4(1:2) b5(1:2)];
        lik(n_converge,:)=[lik1 lik2 lik3 lik6 lik4 lik5];
        rho(n_converge,:)=[rho1 rho2 rho3 rho6 rho4 rho5];
        tau(n_converge,:)=[tau1 tau2 tau3 tau6 tau4 tau5];
    end
