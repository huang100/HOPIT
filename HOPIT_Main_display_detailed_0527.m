...
Pars.Gamma=[0, 10, 10;
            2, 1, 1]';                                                  % Coefficients of cut points
%Pars.Gamma=[-10, 10, 10]';   
...
%     Data.Vignette=[ones(Pars.data_point,1),Vignette]; % It is necessary to add a constant term since Vignette only take values 1-4
    ...
    %
    pred_cut51=Data.Vignette*b(kb+1:kb+kt);
    pred_cut52=pred_cut51+exp(Data.Vignette*b(kb+kt+1:kb+2*kt));
    ...
    if converge==1                                                              % converge==1 if all above models converge
        ...
        AIC(n_converge,:)=[AIC1 AIC2 AIC3 AIC4 AIC5];
        coef(:,:,n_converge)=[b1(1:2) b2(1:2) b3(1:2) b4(1:2) b5(1:2)];
        lik(n_converge,:)=[lik1 lik2 lik3 lik4 lik5];
        rho(n_converge,:)=[rho1 rho2 rho3 rho4 rho5];
        tau(n_converge,:)=[tau1 tau2 tau3 tau4 tau5];
    end
